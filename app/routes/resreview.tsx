...
export default function ResourceReviewPage() {
  ...
  const selectedPull = pulls.find((pull) => pull.number === selectedNumber);
  const selectedComments = selectedNumber ? commentsByPr[selectedNumber] ?? [] : [];
  const selectedStatus = deriveReviewStatus(selectedComments);
  ...
  const loadPulls = async () => {
    ...
    try {
      ...
      setPulls(list);
      setSelectedNumber((current) => current ?? list[0]?.number ?? null);
  ...
  useEffect(() => {
    if (selectedNumber) void loadDetail(selectedNumber);
  }, [selectedNumber]);
  ...
  const addNeedFix = async () => {
    if (!selectedNumber || !needFixMessage.trim()) return;
    try {
  ...
  const markFixed = async (id: string) => {
    ...
    try {
      await createPullRequestComment(selectedNumber, `[ABCC_FIXED_${id}] 已确认修复`);
      await loadDetail(selectedNumber);
  ...
  const addGeneralComment = async () => {
    if (!selectedNumber || !generalComment.trim()) return;
    try {
      await createPullRequestComment(selectedNumber, generalComment.trim());
      setGeneralComment("");
      await loadDetail(selectedNumber);
      toast.success("评论已发送");
    } catch (err) {
  ...
  const approve = async () => {
    if (!selectedNumber) return;
    try {
      await approvePullRequest(selectedNumber);
    ...
    <div className="h-full overflow-hidden px-4 py-5 md:px-6">
      <div className="mx-auto flex h-full max-w-[1500px] flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-white/60">
              {COMMUNITY_REPO_CONFIG.owner}/{COMMUNITY_REPO_CONFIG.name} · {permission}
            </p>
          </div>
        ...
        <div className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[420px_1fr]">
                ...
                visiblePulls.map((pull) => {
                  ...
                  return (
                    <button
                      ...
                      className={`mb-2 flex w-full flex-col gap-2 rounded-xl border px-3 py-3 text-left transition ${
                        selectedNumber === pull.number
          ...
          <section className="min-h-0 overflow-y-auto rounded-2xl border border-white/10 bg-nav-item p-4">
            {!selectedPull ? (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="mb-2 flex items-center gap-2">
                      ...
                      <StatusBadge state={selectedStatus.state} />
                    </div>
                    <h2 className="text-xl font-semibold text-white">{selectedPull.title}</h2>
                    <p className="text-sm text-white/50">
                      {selectedPull.user?.login} · {selectedPull.head.repo?.full_name ?? selectedPull.head.ref} · {selectedPull.head.sha.slice(0, 7)}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="soft" onClick={() => openUrl(selectedPull.html_url)}>
                      在 GitHub 打开
                    </Button>
                    ...
                    <Panel title="改动文件">
                      {loadingDetail ? (
                        ...
                        <div className="flex flex-col gap-2">
                          {files.map((file) => (
                            <div key={file.filename} className="rounded-lg border border-white/10 bg-black/20 p-3">
                              <div className="flex flex-wrap items-center gap-2 text-sm">
                                ...
                                {file.blob_url && (
                                  <button className="ml-auto text-xs text-blue-200 underline" onClick={() => openUrl(file.blob_url!)}>
                                    查看文件
                                  </button>
                                )}
                              </div>
                          ...
                          {files.length === 0 && <p className="text-sm text-white/45">暂无文件信息</p>}
                        </div>
                      )}
                    </Panel>
                    <Panel title="评论流">
                      <div className="flex flex-col gap-2">
                        ...
                        {selectedComments.length === 0 && <p className="text-sm text-white/45">暂无评论</p>}
                      </div>
                    </Panel>
                  </div>
